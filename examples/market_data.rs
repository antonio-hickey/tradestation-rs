...
async fn main() -> Result<(), Error> {
    ...
    //--
    // Example: Stream market depth aggregates on December 2024 Natural Gas Futures
    // (`"NGZ24"`).
    let depth_levels: i32 = 10;
    let streamed_market_depth_quoutes = client
        .stream_market_depth_quotes("NGZ24", Some(depth_levels), |stream_data| {
            // The response type is `responses::MarketData::StreamQuotesResp`
            // which has multiple variants the main one you care about is `Quote`
            // which will contain option chain data sent from the stream.
            match stream_data {
    ...
    // market depth quotes
    println!("{streamed_market_depth_quoutes:?}");
    //--
